...
| Immolation Aura **charge state** (the "capped" signal) | **readable, in BOTH directions** | R6 (+ R7 identity safety) | `observations.md` (OBS-066, Conflagrate) — Settled; `GetSpellCharges().isActive` is annotated `NeverSecret` | Immolation Aura / Consuming Fire "don't cap charges" row | ⚠ **Re-grounded 2026-08-14 from `currentCharges` onto `isActive`.** The old reading — plain iff at max, secret below — made "below max" an unknown; `isActive` is `NeverSecret` and answers both ways, so below-max is a *state* (recharge running) and carries the red `blocked` badge while max carries the gold `capped` one. What `isActive` still cannot say is *which* charge is recharging (true at both 1/2 and 0/2), which is why "about to cap" remains unexpressible. R7 is about reading the right spell's charges across the flip. A refused read is still no cue at all. |
...
| **Eye Beam cooldown remaining** (≤4s — a sealed duration) | **sealed-display** (`sealed-duration-range`) | S4 (range step-curve on a duration object → texture alpha) | `cdm-rider-patterns.md` (`Channel.Threshold`) — Settled | **Cue C2.** Essence Break sealed hold marker (don't clip the amp window into Eye Beam) | Curve reads ~true only while Eye Beam's cooldown-remaining sits inside the authored ≤4s band, piped into Essence Break's hold-texture alpha. Duration is Eye Beam's own cooldown object; guard the Part-3 curve feature-gate. Lua never reads the clock. |
...
| Readable **dependency dots** — a related ability's cooldown state | **readable** | R2 + R7 (composed) | R2: `cooldown-manager.md`, `cdm-rider-patterns.md`; R7: `cooldown-manager.md`, `observations.md` — Settled | **Cue C1.** Metamorphosis reset dots (Eye Beam + Death Sweep) & The Hunt sync dot (Meta) | Readable cooldown states, no sealed value. Green = the related ability is on cooldown, red = ready. Meta's two dots say its reset banks a cast (green) or is wasted (red); The Hunt's dot says cast now (Meta on cooldown, green) or hold for the Meta window (red). Shown only while the parent is castable / not transformed. Reuses Demonology's readable-marker shape — no new renderer, possibly two markers per row. |
...
- **Eye Beam cooldown remaining ≤4s** (`sealed-duration-range`, S4 → cue C2) → a **range step-curve
  on Eye Beam's cooldown duration object → texture alpha** on **Essence Break's** hold marker (don't
  clip the amp window into Eye Beam). Lua never reads the clock; the curve result goes straight to
  the draw call. Reported `offered` / `armed` / `refused`.
...
5. **Buff-maintenance marker readability (Exergy / Serrated Glaive).** Question: does a self-buff
   ...
   the `scenarios.md` walk (ST-1 Vengeful Retreat / Exergy, ST-10 Throw Glaive / Serrated Glaive).
