...
`sealed-power-percent`, `sealed-cooldown-range`. ⚠ `../spec.md` §3.6 calls the last one
`sealed-duration-range`, which nothing accepts; the mismatch is a tracked follow-up and is not
fixed here.
...
**A structural limit this pass made concrete: a marker is readable *or* sealed, never both, and
two markers union (OR) rather than intersect (AND).** So a condition that ANDs a readable fact
with a sealed one cannot be authored at all. Three do — The Hunt's rung-4 hold, Vengeful Retreat's
rung-5 alignment gate, and Metamorphosis's rung-3 condition. The first two are authored as the
**actionable slice** of the real condition and marked as such; the third splits cleanly, because
its two halves are an OR (`meta_wastes_eye_beam` ∪ `meta_awaits_eye_beam` ∪
`meta_wastes_death_sweep`) rather than an AND of a readable and a sealed term.
