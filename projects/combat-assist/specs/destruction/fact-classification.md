...
| **Backdraft application count** | **sealed-display** | S2 → `player-aura-stacks` | OBS-065 (`observations.md:1161`) measured this aura: 1 stack = icon and swipe only, 2 = the number "2". ⚠ **The `min = 2` guard was RETIRED with `player-aura-stacks` on 2026-08-22** (`../backlog.md`) — it was never a ceiling; `sealed-count-bands` authors the threshold as cap's own rule. The `aoe_dia` rung 13 threshold is expressible today | **nothing in this catalog.** The single-target rungs ask `stack<1`, which the boolean answers. Kept here because a Hellcaller or AoE-deep catalog will want it |
...
| `variable.ritual_length` | **sealed, no authored form** | — | a sum of three Diabolic Ritual auras' remaining times. ⚠ **"there is no aura-remaining band" was true when written and is now FALSE** (2026-09-01): V19's outside-window hatch rides band tables on an aura's remaining *seconds*, and Protection ships one (`outside_s: 6`). What survives is the harder half — this fact is a **sum** of three auras' remainings, which no single-subject band can take | rungs 3 and 13. **No hint.** §5.2 |
