...
- **Ordering recovers instead of latching** (2026-08-17). Three defects made a scrambled row
  ...
  - **The panel's second row is real** (2026-08-31). The panel has been `6x2` since it was
    ...
    - **Havoc is at twelve of twelve cells.** One more authored entry needs a wider panel
      ...
      not a code change.
    - **Flown 2026-08-31**, over a week of play on Havoc — which is the spec at twelve of
      twelve cells, so the second row was on screen the whole time. The two things the tests
      could not reach both held: the second row drew where the arithmetic says, and parked
      frames stayed out of the reading sort. `X{MISMATCH}` is the standing signal for the
      latter — parked frames are absent from `P.tracked` and would sort ahead of the first row
      if a future edit walked `P.claimed` instead, so the check keeps its subject.
