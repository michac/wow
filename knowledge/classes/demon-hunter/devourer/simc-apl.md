---
title: Devourer Demon Hunter — simc APL (generated)
patch: 12.1
fetched: 2026-08-17
reviewed: 2026-08-17
sources:
  - https://raw.githubusercontent.com/simulationcraft/simc/midnight/engine/class_modules/apl/apl_demon_hunter.cpp  # tier 1, simc APL source for devourer
verbatim: true
confidence: high
pass: 2026-08-17.1
---

# Devourer Demon Hunter — the simc priority list
...
<!-- gen_simc:begin -->
_Generated by `wowkb.simc --kb` from `engine/class_modules/apl/apl_demon_hunter.cpp` (`devourer` block), commit unresolved (`--no-sha`, or the API was rate-limited). 157 actions. Do not hand-edit._
...
actions.default+=/run_action_list,name=annihilator_melee,if=hero_tree.annihilator&talent.the_hunt
actions.default+=/run_action_list,name=annihilator_ranged,if=hero_tree.annihilator
actions.default+=/run_action_list,name=voidscarred_ranged,if=hero_tree.voidscarred&!talent.the_hunt
actions.default+=/run_action_list,name=voidscarred_melee,if=hero_tree.voidscarred&talent.the_hunt
...
actions.annihilator_ranged+=/call_action_list,name=reaps,if=!buff.metamorphosis.up&action.reap.souls_consumed>=4&variable.wont_drop_meta
actions.annihilator_ranged+=/collapsing_star,if=active_enemies=1
actions.annihilator_ranged+=/call_action_list,name=reaps,if=active_enemies>1&buff.eradicate.up&action.reap.souls_consumed>=1+3*set_bonus.midnight_season_2_4pc&variable.wont_drop_meta
actions.annihilator_ranged+=/call_action_list,name=reaps,if=action.reap.souls_consumed>=4
actions.annihilator_ranged+=/call_action_list,name=reaps,if=fight_remains<=6&action.reap.souls_consumed>=1
actions.annihilator_ranged+=/call_action_list,name=reaps,if=talent.eradicate&active_enemies>1&buff.eradicate.up&action.reap.souls_consumed>=1+3*set_bonus.midnight_season_2_4pc
...
actions.vsm_meta+=/throw_glaive,if=variable.void_ray_count>=2&fury>void_metamorphosis_base_drain_ps*gcd.max*2&cooldown.void_ray.remains>gcd.max
...
actions.vsm_meta+=/eradicate,if=(action.reap.souls_consumed>=10|buff.moment_of_craving.remains<gcd.max|cooldown.void_ray.remains<gcd.max)&!buff.soulburst.up
...
actions.vsm_out+=/eradicate,if=action.reap.souls_consumed>=10&buff.void_metamorphosis_stack.stack<buff.void_metamorphosis_stack.max_stack-3&!buff.soulburst.up
actions.vsm_out+=/void_ray,if=talent.eradicate&!buff.eradicate.up
actions.vsm_out+=/consume,if=buff.soulburst.up
