...
| **Vengeful Retreat** | Movement / Defensive | — | ~25s CD (charge ability — the generated file reads the GCD here) | Removes all snares and vaults you backward, damaging nearby enemies `[T1]`. ⚠ The pre-12.1 claim that it is "buffed by *Voidstep*" is **removed: no talent named Voidstep exists in the 12.1 tree.** What buffs it now is **Hungering Slash**, which since 12.1 grants a temporary extra charge and empowers the next retreat within 6s `[T1]`; *Vengeful Bonds* snares nearby enemies by 70% for 3s `[T1]`. |
