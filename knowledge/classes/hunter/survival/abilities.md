...
> measured. One caveat: the tsv's `cooldown` column returns the **GCD** for
> charge-based abilities (Harpoon reads `1`), so a sub-2-second value there is not a
> real recharge. This table is for **function, role and rotational context**.
