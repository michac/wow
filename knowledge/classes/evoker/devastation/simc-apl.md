---
title: Devastation Evoker — simc APL (generated)
patch: 12.1
fetched: 2026-08-17
reviewed: 2026-08-17
sources:
  - https://raw.githubusercontent.com/simulationcraft/simc/midnight/engine/class_modules/apl/apl_evoker.cpp  # tier 1, simc APL source for devastation, commit 5a6a0e1 (2026-08-11)
verbatim: true
confidence: high
---

# Devastation Evoker — the simc priority list
...
<!-- gen_simc:begin -->
_Generated by `wowkb.simc --kb` from `engine/class_modules/apl/apl_evoker.cpp` (`devastation` block), commit `5a6a0e1` (2026-08-11). 86 actions. Do not hand-edit._
...
actions.default+=/potion,if=(!talent.dragonrage|buff.dragonrage.up)|fight_remains<35
actions.default+=/variable,name=next_dragonrage,value=cooldown.dragonrage.remains<?((cooldown.eternity_surge.remains-8)>?(cooldown.fire_breath.remains-8))
actions.default+=/invoke_external_buff,name=power_infusion,if=buff.dragonrage.up|fight_remains<35
actions.default+=/variable,name=can_use_empower,op=set,value=cooldown.dragonrage.remains>=gcd.max*variable.dr_prep_time,if=talent.animosity&talent.dragonrage
actions.default+=/variable,name=use_pyre_fs,op=set,value=talent.legacy_of_the_lifebinder&(active_enemies>=5|active_enemies>=3&(talent.volatility.rank=2|talent.feed_the_flames))
actions.default+=/variable,name=use_tts,op=set,value=(cooldown.eternity_surge.remains<=action.fire_breath.usable_in),if=talent.legacy_of_the_lifebinder
actions.default+=/quell,use_off_gcd=1,if=target.debuff.casting.react
actions.default+=/call_action_list,name=trinkets
actions.default+=/run_action_list,name=sc,if=talent.mass_disintegrate
actions.default+=/run_action_list,name=fs
...
actions.sc+=/pyre,target_if=max:target.health.pct,if=(active_enemies>=5|active_enemies>=4&talent.volatility.rank=2)&!buff.mass_disintegrate_stacks.up|active_enemies>=3&talent.charged_blast&buff.charged_blast.stack>=15
actions.sc+=/disintegrate,target_if=max:dot.fire_breath_damage.remains,if=(raid_event.movement.in>2|buff.hover.up),early_chain_if=ticks_remain<=1,interrupt_if=ticks_remain<=1&active_enemies>=3
actions.sc+=/living_flame,if=buff.burnout.up&buff.leaping_flames.up&active_enemies<=1+buff.leaping_flames.stack
actions.sc+=/unbound_flame,if=!buff.essence_burst.at_max_stacks
actions.sc+=/azure_sweep
actions.sc+=/living_flame,if=raid_event.movement.in>execute_time|buff.hover.up
