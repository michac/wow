---
title: Arms Warrior — simc APL (generated)
patch: 12.1
fetched: 2026-08-17
reviewed: 2026-08-17
sources:
...
<!-- gen_simc:begin -->
_Generated by `wowkb.simc --kb` from `engine/class_modules/apl/apl_warrior.cpp` (`arms` block), commit `f61263a` (2026-08-13). 146 actions. Do not hand-edit._
...
actions.default+=/potion,if=gcd.remains=0&debuff.colossus_smash.remains>8|target.time_to_die<25
...
actions.default+=/arcane_torrent,if=cooldown.mortal_strike.remains>1.5&rage<50
actions.default+=/lights_judgment,if=debuff.colossus_smash.down&cooldown.mortal_strike.remains
actions.default+=/bag_of_tricks,if=debuff.colossus_smash.down&cooldown.mortal_strike.remains
actions.default+=/berserking,if=target.time_to_die>180&debuff.colossus_smash.up|target.time_to_die<180&variable.execute_phase&debuff.colossus_smash.up|target.time_to_die<20
actions.default+=/blood_fury,if=debuff.colossus_smash.up
actions.default+=/fireblood,if=debuff.colossus_smash.up
actions.default+=/ancestral_call,if=debuff.colossus_smash.up
actions.default+=/invoke_external_buff,name=power_infusion,if=debuff.colossus_smash.up&fight_remains>=135|variable.execute_phase&buff.avatar.up|fight_remains<=25
actions.default+=/run_action_list,name=colossus_aoe,if=talent.demolish&active_enemies>2
actions.default+=/run_action_list,name=colossus_execute,target_if=min:target.health.pct,if=talent.demolish&variable.execute_phase
actions.default+=/run_action_list,name=colossus_st,if=talent.demolish
actions.default+=/run_action_list,name=slayer_aoe,if=talent.slayers_dominance&active_enemies>2
actions.default+=/run_action_list,name=slayer_execute,target_if=min:target.health.pct,if=talent.slayers_dominance&variable.execute_phase
actions.default+=/run_action_list,name=slayer_st,if=talent.slayers_dominance
...
actions.colossus_execute+=/mortal_strike,if=buff.colossal_might.stack<10|buff.executioners_precision.stack=2&buff.battlelord.remains
...
actions.colossus_st+=/rend,if=!talent.cleave&dot.rend_dot.remains<=gcd|cooldown.colossus_smash.remains<2&dot.rend_dot.remains<=10
