...
sources:
  ...
  - ./simc-apl.md  # tier 1, GENERATED by wowkb.simc --kb from engine/class_modules/apl/warlock.cpp (`destruction` block), commit 8ec56ea (2026-08-18) — the priority list of record; this file augments it
...
`wowkb.simc --kb` from simc's `warlock.cpp` **`destruction` block, commit
`8ec56ea` (2026-08-18)**, i.e. post-12.1. **This file augments that list; it does
not replace it** — what lives here is the *why*, plus method.gg (Tier 3) for
colour. When the two disagree, `simc-apl.md` wins and this file is what needs
fixing.

The APL branches by **hero tree** — `actions.aoe_dia` (Diabolist / Diabolic
Ritual) and `actions.aoe_hc` (Hellcaller / Wither), each gated on
`active_enemies>=2` — and `actions.default` is the shared single-target list.
Destruction is a **Soul Shard economy**: keep your fire DoT up, never overcap
shards, and pour shards into **Chaos Bolt** (single target) or **Rain of Fire**
(big AoE), timed around **Summon Infernal**.
...
The annotated reading of `actions.default` in `./simc-apl.md`. The conditions
themselves live there and are not restated in full.

1. **Soul Fire** if `soul_shard<=4` (fits without overcapping)
2. **Conflagrate at 2 charges** — a pure don't-sit-on-a-charge rung, and it sits
   *above* the Chaos Bolt spend. ⚠ **This rung is new to the reading here**: it
   existed in the module all along but the earlier distillation only carried the
   lower Conflagrate. It is why "don't sit on 2 charges" is a rule and not a
   nicety.
3. **Chaos Bolt** to spend a **Demonic Art** proc or restart Diabolic Ritual
   (`demonic_art` up, or ritual shorter than the cast, and target >20% HP)
4. **Conflagrate** to build if `soul_shard<=4.2` and Backdraft is not stacked
5. **Summon Infernal**
6. **Malevolence** (Hellcaller only — the rung is in the shared list and is dead
   on a Diabolist build)
7. **Shadowburn** with a free **Fiendish Cruelty** proc up, or when
...
8. **Wither** (Hellcaller) / **Immolate** (Diabolist) — refresh in the pandemic
   window (<30% duration / refreshable), with the Internal Combustion and
   Cataclysm/Soul Fire interlocks the APL line spells out
9. **Conflagrate** again, at the looser `soul_shard<=4.4` gate
10. **Ruination** (free proc — press it)
11. **Cataclysm** if **Lake of Fire** talented
12. **Chaos Bolt** as the main shard dump (ritual length >4)
13. **Infernal Bolt** if `soul_shard<=3` (shard refill)
14. **Incinerate** (filler)

⚠ **The Chaotic Inferno Incinerate rung is gone.** An earlier reading of this
list carried *"Incinerate if `buff.chaotic_inferno_buff.up` and
`soul_shard<=4.6`"* between Summon Infernal and Shadowburn. That line is **not
in the 12.1 module** (`8ec56ea`) and has been deleted rather than annotated —
Incinerate is filler here and nothing promotes it.
...
**Diabolist (`aoe_dia`, gated `active_enemies>=2&talent.diabolic_ritual`):**
1. **Summon Infernal**
2. **Chaos Bolt** for Demonic Art / short ritual, up to
   `active_enemies<=(10 - 2 × talent.destructive_rapidity)` — Diabolist keeps
   pressing Chaos Bolt a long way into AoE. ⚠ **This gate widened at `8ec56ea`**
   (it read `active_enemies<=4` in the pre-12.1 reading), and the health clause
   `target.health.pct>20` that used to ride it is gone.
3. **Rain of Fire** at **3+ targets** when
   `soul_shard >= (3.5 - 0.1 × active_dot.immolate)` or Alythess's Ire up — see
   the ⚠ note below before using the 3.5. ⚠ **This threshold moved from 4 to 3.**
4. **Conflagrate** to refresh Immolate across targets (`target_if` most Immolate
   remaining)
5. **Shadowburn** on low-HP targets, at a target count that widens with a free
   **Fiendish Cruelty** proc and with Conflagration of Chaos
6. **Ruination**
7. **Cataclysm** (Lake of Fire, or when no adds incoming)
8. **Havoc** on the longest-living off-target
9. **Infernal Bolt** if `soul_shard<3`
10. **Chaos Bolt** at **≤2 targets** with long ritual (was ≤3)
...
>
> ```
> aoe_dia  rain_of_fire,if=((soul_shard>=(3.5-0.1*(active_dot.immolate)))|buff.alythesss_ire.up)&active_enemies>=3
> aoe_hc   rain_of_fire,if=(soul_shard>=(4.0-0.1*(active_dot.wither)))&active_enemies>=(5-talent.destructive_rapidity)   ← Hellcaller, 4.0
> aoe_hc   rain_of_fire,if=active_enemies>=(5-talent.destructive_rapidity)                                               ← NO shard condition at all
> ```
>
...
> *(Re-read 2026-08-19 off `./simc-apl.md` @ commit `8ec56ea`; first noticed in the
> local simc checkout `raw/addon-research/simc` @ `ab7b0b8`, 2026-08-01. The
> constants survived 12.1 unchanged — only the Diabolist line's target gate moved,
> 4 → 3.)*
...
**The numbered lists above annotate `./simc-apl.md`**, generated from simc's
`warlock.cpp` `destruction` block at commit **`8ec56ea` (2026-08-18)** — which
post-dates both the 12.1 launch and its tuning pass.
`uv run python -m wowkb.simc --kb warlock destruction --check` is the gate and it
passes. The ordering is therefore Tier-1 current, which is why `confidence:` is
`high`.

Two things that are current here are **not** current elsewhere, and conflating
them is the trap:

- **`sims.md` is still pre-12.1.** A module APL carries no profileset results, so
  it settles the *order* and says nothing about DPS, stat weights, or the
  hero-tree margin.
- **The Tier-3 guide captures in this directory** (`maxroll-*.md`, and the
  method.gg pages cited above) were written pre-12.1. Where they disagree with
  the APL, the APL wins.

## TODO

- [x] **Re-distill off a 12.1 simc APL** — done 2026-08-19. The list of record is
      `./simc-apl.md` (generated, Tier 1, `warlock.cpp` `destruction` block,
      commit `8ec56ea` 2026-08-18); this file annotates it. Confidence raised to
      `high`.
- [ ] **Regenerate `sims.md`** at 12.1 — the APL is current, the numbers are not.
