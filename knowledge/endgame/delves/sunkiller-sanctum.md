...
## T8 tips (Affliction warlock, ~242)

T8 is a comfortable clear for you (you do T9 rough). The boss is the
only real check, and it's an interrupt/add fight — which Affliction
handles well:
...
- **Don't bother dispelling Coalescing Malediction** even if you could
  ...
  add; heal/leech through the DoT instead.
- **Before he casts Gorge, sweep the field** — make sure no Voidcaller
  ...
  fall off before re-engaging.
- Grab **Core Empowered** orbs (Core of the Problem variant) — the DR
  stacks are free survivability.

## TODO

- [ ] Trash pack specifics (neither tier-3 source detailed them)
