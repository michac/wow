---
title: Hallick – Kil'jaeden (US) — druid alt snapshot
patch: 12.0.7
fetched: 2026-06-19
sources:
  - https://us.api.blizzard.com/profile/wow/character/kiljaeden/hallick?namespace=profile-us
confidence: high
---

# Hallick – Kil'jaeden (US)

> **Snapshot** — character data is volatile (updates on logout). The body
> below reflects the **2026-06-03** profile fetch; front matter was
> re-stamped to 12.0.7 (2026-06-19) WITHOUT a live profile re-fetch.
> **Always re-fetch the live profile before answering any gear/level/progress
> question** — level, ilvl, spec, and Midnight progression may all have moved.

**Alt of the user's main (Encomplete)** — same guild (The Soggy Bottom
Boys), achievement points match account-wide (7,100 vs 7,125).

## Identity

- Night Elf **Druid**, active spec **Feral**
- Level **80** (TWW cap — has not started Midnight; cap is 90)
- Alliance, Kil'jaeden · Title: Knight-Lieutenant
- Last login at fetch: 2026-06-02 (active)

## State

- Equipped ilvl **102** — pre-Midnight leveling gear
- Needs the 80→90 Midnight leveling path before any endgame loop
  (weekly checklist, delves, crests) applies

## Talent audit (2026-06-03)

- Has a full Feral loadout (TWW carryover) but **NO hero talent tree
  selected** — flagged to user; rec is **Druid of the Claw** for
  leveling (Ravage AoE; Icy Veins/Maxroll 12.0.5).
- Leveling rotation page: `../classes/druid/feral/leveling-rotation.html`
  (built from his live loadout + tooltips, `raw/blizzard/hallick-specializations.json`)

## TODO

- [ ] Leveling route 80→90 (Midnight zones; check Icy Veins leveling guide)
- [x] Feral leveling rotation page created 2026-06-03; full max-level
      build/rotation docs still TODO when he hits 90
- [ ] Heirloom / warband XP buffs in 12.0 — worth fetching if user asks
