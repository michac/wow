...
- **Hero trees:** `builds.md` owns the pick. In kit terms: **Oracle**'s live
  subtree carries **Prophet's Insight**, **Prophet's Will**, **Piety** and
  **Twinsight**, and the old `Premonition` button is **gone** (see the
  reconciliation notes); **Voidweaver** adds `Entropic Rift` / `Void Blast` /
  `Void Torrent`. ⚠ `builds.md` still describes an Oracle "Premonition toolkit"
