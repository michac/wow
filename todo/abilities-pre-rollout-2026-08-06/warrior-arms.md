...
- **Slayer** — adds no button; `Slayer's Dominance` applies *Marked for
  Execution* and the branch leans hard on `Execute` + `Sudden Death`.
- **Colossus** — adds the channelled finisher `Demolish`, empowered by stacking
  `Colossal Might`.
...
>
> ⚠ One deliberate exception to "the tsv wins" is flagged in the `Heroic Strike`
> row below.
