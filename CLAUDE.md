...
uv run python -m wowkb.kblint                           # the 6 KB gates: 1-3 + 6 = README §7's current-state rule, 4-5 = §0's evidence classes. **1, 3 and 6 are KB-WIDE** (`--all`); 2, 4 and 5 stay scoped to knowledge/addon-dev, whose evidence classes are stricter — gate 2 would fire on every legitimate game date in the gameplay KB. Gate 6 = no correction in the LEDE (the text above the first `##`, which is what a reader who stops after the first screen sees) — scoped to retrospective prose only, so a lede warning about the CURRENT state is fine. Gate 4 = no negative existential ("there is no way to…") in a claim unit tagged [client] — a negative takes [searched YYYY-MM-DD: <instruments>], which names where we looked. Gate 5 = no claim citing OBS-nnn / projects/** / a capture path / one of OUR addons in any of its three names: repo (CDMProbe), slash command (/cdmp, /clab, /bb, /ps, /cap) or capture reader (wowkb.capture/.cdmp/.lab/.obs/.addon/.diagnostics) — but NOT wowkb.uiapi/.wiki/.wago, which reach admissible sources. `--gate N` for one; exit 1 on any hit
uv run python -m wowkb.citecheck                        # **do the Tier-1 source citations still resolve?** A companion to `kblint`, deliberately separate because its subject is the citation rather than the claim. It reads every `[T1 src …]` / `[T1 docs …]` in `knowledge/addon-dev/`, resolves the named file in the matching `raw/addon-research/` clone (`@12.1*` → the 12.1 clone, anything else → the 12.0.7 one), and greps it for the cited **symbol**. A symbol citation that does not resolve is a hard failure (exit 1) — the claim above it is describing code that is gone. A **line-anchored** citation is counted and never failed: there are 1174 of them, they predate the rule, and `_meta/kb-inbox.md` carries the repair job. The rule exists because a line anchor is right in the files Blizzard did not touch and silently wrong in the ones it reworked, with nothing in the citation to tell them apart — measured 2026-08-21: `Dump.lua` is 486 lines on both 12.0.7.68887 and 12.1.0.69273 with `type(val)` at 98/149/309 in both, while `CooldownViewer.lua` went 2168 → 2374. **Write new citations symbol-anchored**, with a verbatim quoted fragment where the wording matters; `--lines` lists the legacy locators, `--verbose` the resolved ones
```
